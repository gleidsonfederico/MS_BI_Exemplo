...
SELECT X.[Cod_Cliente]
      ...
      ,X.[Quantidade]
      ,X.[Unidades_Vendidas]
	  ,(X.[Quantidade]/Y.[Quantdade_Vendida_002]) * Y.[FRETE] AS [Frete Rateio]
	  ,(X.[Quantidade]/W.[Quantdade_Vendida_003]) * W.[Custo_Fixo] AS [Custo Fixo Rateio]
	  ,(X.[Quantidade]/Z.[Quantdade_Vendida_004]) * Z.[Meta_Faturamento] AS [Meta Faturamento Rateio]
	  ,(X.[Quantidade]/K.[Quantdade_Vendida_005]) * K.[Meta_Custo] AS [Meta Custo Rateio]
...
SELECT [Cod_Cliente]
      ...
      ,[Cod_Fabrica]
      ,[Cod_Dia]
      ,SUM([Quantidade]) AS Quantdade_Vendida_002
...
INNER JOIN (
  ...
  INNER JOIN (
   SELECT [Cod_Fabrica]
      ,[Cod_Dia]
      ,SUM([Quantidade]) AS Quantdade_Vendida_003
  FROM [DW_SUCOS].[dbo].[Fato_001]
...
inner join (
  ...
  INNER JOIN (
      ...
      ,[Cod_Organizacional]
      ,[Cod_Dia]
      ,SUM([Quantidade]) AS Quantdade_Vendida_004
  FROM [DW_SUCOS].[dbo].[Fato_001]
  GROUP BY [Cod_Cliente]
      ,[Cod_Produto]
      ,[Cod_Organizacional]
...
INNER JOIN (
  ...
  INNER JOIN (
      ...
      ,[Cod_Fabrica]
      ,[Cod_Dia]
      ,SUM([Quantidade]) AS Quantdade_Vendida_005
  FROM [DW_SUCOS].[dbo].[Fato_001]
